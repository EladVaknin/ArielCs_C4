...
boolean insert(char *word) 
{
	...
	while (word[counter] != '\0') {
		...
		if (PointNode->children[index] == NULL) {//if char doesn't exist - create new node and set letter
			PointNode->children[index] = init(); //and init
			if (PointNode->children[index] == NULL)
				return FALSE;	//fail to allocate memory
			PointNode->children[index]->letter = word[counter]; //place it
		}
	...
	while (word[i] != EOF) {
		...
		if (word[i] == ' ' || word[i] == '\n') {
			...
			if (insert(word) == FALSE) {	//add word to tree
				printf("failed to add the word %s to the trie", word);
				free(word);
				return;
			}
...
void printTrie(boolean r) {
	...
	if (r == FALSE) {	//print in lexicographically order
...
void killTree(node *root) { //free memory of tree
	if (root != NULL) {
		for (int i = 0; i < NUM_LETTERS; i++) //free children first
			killTree(root->children[i]); //in recursion
		free(root);
	}
...
void printTreeR(node *PointNode, char *word, int size) {
	char *temp = NULL;
	if (PointNode == NULL)	//check if the node is empty - nothing to print
		return;
	word[size - 2] = PointNode->letter;	//add current char to string and end word
	word[size - 1] = '\0';
	size++;
	temp = malloc(size);	//allocate for next children
	if (temp == NULL)
		return;
	...
	if (PointNode->count > 0)	
		printf("%s\t%ld\n", word, PointNode->count);//end of word - print word
}